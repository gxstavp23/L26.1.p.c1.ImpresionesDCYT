import { I_vSolicitud } from "../interfaces/I_vSolicitud.js";
import Cl_sSolicitud from "../services/Cl_sSolicitud.js";
import Cl_mSolicitud from "../models/Cl_mSolicitud.js";
import Cl_mCarrito from "../models/Cl_mCarrito.js";

export default class Cl_cSolicitud {
  private vista: I_vSolicitud;
  private servicio: Cl_sSolicitud;
  private modelo: Cl_mCarrito;

  constructor(vista: I_vSolicitud, servicio: Cl_sSolicitud) {
    this.vista = vista;
    this.servicio = servicio;
    this.modelo = new Cl_mCarrito();

    // Renderiza el carrito y la tasa del dia
    this.vista.mostrarTasaDelDia(this.modelo.getTasa());
    this.actualizarVistaCarrito();

    // Enlazamos los eventos de la vista con los metodos del controlador
    this.vista.bindAddImpresion(this.handleAddImpresion.bind(this));
    this.vista.bindAddDescarga(this.handleAddDescarga.bind(this));
    this.vista.bindAddArticulo(this.handleAddArticulo.bind(this));
    this.vista.bindEliminarItem(this.handleEliminarItem.bind(this));
    this.vista.bindModificarItem(this.handleModificarItem.bind(this));
    this.vista.bindEnviarPedido(this.handleEnviarPedido.bind(this));
    this.vista.bindConfirmPayment(this.handleConfirmPayment.bind(this));

    this.vista.bindVerProductos?.(this.handleVerProductos.bind(this));
    this.vista.bindAgregarProducto?.(this.handleAgregarProducto.bind(this));
    this.vista.bindEliminarProducto?.(this.handleEliminarProducto.bind(this));
    this.vista.bindHacerPedido?.(this.handleHacerPedido.bind(this));
  }
  
  //si hay algun cambio, esta vaina actualiza el carro y lo renderiza, 
  private actualizarVistaCarrito() {
    this.vista.renderizarCarrito(
      this.modelo.getItems(), 
      this.modelo.getTotalBs(),
      this.modelo.getTotalUsd(),
      this.modelo.getTasa()
    );
  }

  private handleAddImpresion(): void {
    const datos = this.vista.getDatosImpresion();
    if (!datos.nombre) {
      this.vista.mostrarMensaje("Debe ingresar el nombre del archivo a imprimir.", "error");
      return;
    }
    this.modelo.agregarImpresion(datos.nombre, datos.hoja, datos.formato, datos.copias);
    this.vista.limpiarInputsImpresion();
    this.actualizarVistaCarrito();
  }

  private handleAddDescarga(): void {
    const datos = this.vista.getDatosDescarga();
    if (!datos.enlace) {
      this.vista.mostrarMensaje("Debe ingresar el enlace de descarga.", "error");
      return;
    }
    this.modelo.agregarDescarga(datos.enlace);
    this.vista.limpiarInputsDescarga();
    this.actualizarVistaCarrito();
  }

  private handleAddArticulo(): void {
    const datos = this.vista.getDatosArticulo();
    if (datos.cantidad <= 0) {
      this.vista.mostrarMensaje("La cantidad debe ser mayor a 0.", "error");
      return;
    }
    this.modelo.agregarArticulo(datos.tipo, datos.cantidad);
    this.vista.limpiarInputsArticulo();
    this.actualizarVistaCarrito();
  }

  private handleEliminarItem(id: string): void {
    this.modelo.eliminarItem(id);
    this.actualizarVistaCarrito();
  }

  private handleModificarItem(id: string, accion: 'incrementar' | 'decrementar'): void {
    const delta = accion === 'incrementar' ? 1 : -1;
    this.modelo.modificarCantidadItem(id, delta); // El modelo se encarga de eliminar el item si la cantidad llega a 0
    this.actualizarVistaCarrito();
  }

  private ordenIdActual: string | null = null;

  private async handleEnviarPedido(): Promise<void> {
    const cliente = this.vista.getDatosCliente();
    const items = this.modelo.getItems();

    if (!cliente.cedula || !cliente.nombre) {
      this.vista.mostrarMensaje("Por favor complete sus datos personales.", "error");
      return;
    }
    if (items.length === 0) {
      this.vista.mostrarMensaje("El carrito está vacío. Agregue algún servicio antes de enviar.", "error");
      return;
    }

    this.ordenIdActual = this.generarOrdenId();
    this.vista.showPaymentModal(cliente.cedula, this.ordenIdActual);
  }

  private handleVerProductos(): void {
    const servicios = Cl_mCarrito.obtenerServicios();
    this.vista.mostrarProductos?.(servicios);
  }

  private handleAgregarProducto(): void {
    const codigo = this.vista.getCodigoProducto?.();
    const cantidad = this.vista.getCantidadProducto?.() ?? 1;

    if (!codigo) {
      this.vista.mostrarMensaje("Ingrese el código del producto.", "error");
      return;
    }

    const servicio = Cl_mCarrito.buscarServicioPorCodigo(codigo);
    if (!servicio) {
      this.vista.mostrarMensaje(`No se encontró el servicio con código ${codigo} o el servicio no está disponible.`, "error");
      return;
    }

    if (cantidad <= 0) {
      this.vista.mostrarMensaje("La cantidad debe ser mayor a 0.", "error");
      return;
    }

    this.modelo.agregarItemDirecto(servicio.codigo, servicio.nombre, cantidad, servicio.precio);
    this.actualizarVistaCarrito();
  }

  private handleEliminarProducto(): void {
    const codigo = this.vista.getCodigoProducto?.();
    if (!codigo) {
      this.vista.mostrarMensaje("Ingrese el código del producto a eliminar.", "error");
      return;
    }

    this.modelo.eliminarItemPorCodigo(codigo);
    this.actualizarVistaCarrito();
  }

  private async handleHacerPedido(): Promise<void> {
    const cliente = this.vista.getDatosCliente();
    const items = this.modelo.getItems();
    const pago = this.vista.getDatosPago?.();

    if (!cliente.cedula || !cliente.nombre) {
      this.vista.mostrarMensaje("Por favor complete sus datos personales.", "error");
      return;
    }

    if (items.length === 0) {
      this.vista.mostrarMensaje("El carrito está vacío. Agregue algún servicio antes de enviar.", "error");
      return;
    }

    if (!pago || !pago.cedula) {
      this.vista.mostrarMensaje("Ingrese la cédula en el método de pago.", "error");
      return;
    }

    if (pago.tipo === 'movil' && (!pago.referencia4 || !/^[0-9]{4}$/.test(pago.referencia4))) {
      this.vista.mostrarMensaje("Ingrese los últimos 4 dígitos de la referencia para Pago Móvil.", "error");
      return;
    }

    if (pago.tipo === 'transferencia' && (!pago.banco || !pago.telefono)) {
      this.vista.mostrarMensaje("Ingrese banco y teléfono para Transferencia.", "error");
      return;
    }

    if (pago.tipo === 'efectivo') {
      pago.banco = null;
      pago.telefono = null;
      pago.referencia4 = null;
    }

    const ordenId = this.generarOrdenId();
    const clienteConOrden = { ...cliente, codigoOrden: ordenId };
    const dto = {
      ...this.modelo.generarDTO(clienteConOrden),
      pago
    };

    this.vista.mostrarMensaje("Procesando solicitud, por favor espere...", "exito");
    const exito = await this.servicio.enviarSolicitud(dto);

    if (exito) {
      this.vista.mostrarMensaje("¡Solicitud enviada con éxito! Verificaremos su pago en breve.", "exito");
      this.modelo.vaciarCarrito();
      this.vista.limpiarTodo();
      this.actualizarVistaCarrito();
    } else {
      this.vista.mostrarMensaje("Ocurrió un error al enviar la solicitud. Intente nuevamente.", "error");
    }
  }

  private generarOrdenId(): string {
    const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    const bytes = new Uint8Array(5);
    crypto.getRandomValues(bytes); // Genera un código aleatorio de 5 caracteres para la orden
    return Array.from(bytes).map((byte) => chars[byte % chars.length]).join("");
  }


  // Este método se llama cuando el usuario confirma los datos de pago en la vista (modal)
  private async handleConfirmPayment(pago: { tipo: string; cedula: string; banco: string | null;telefono: string | null; referencia4: string | null }): Promise<void> {
    const ordenId = this.ordenIdActual ?? this.generarOrdenId();
    const cliente = { ...this.vista.getDatosCliente(), codigoOrden: ordenId };

    this.vista.mostrarMensaje("Procesando solicitud, por favor espere...", "exito");
    const dto = {
      ...this.modelo.generarDTO(cliente),
      pago
    };

    const exito = await this.servicio.enviarSolicitud(dto);

    if (exito) {
      this.vista.mostrarMensaje("¡Solicitud enviada con éxito! Verificaremos su pago en breve.", "exito");
      //Se llaman estos metodos que son para limpiar la vista entera :)
      this.modelo.vaciarCarrito();
      this.vista.limpiarTodo();
      this.actualizarVistaCarrito();
    } else {
      this.vista.mostrarMensaje("Ocurrió un error al enviar la solicitud. Intente nuevamente.", "error");
    }
  }
}