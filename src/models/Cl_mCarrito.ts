...
export default class Cl_mCarrito {
  ...
  private static readonly servicioStorageKey = "impresiones_services";

  public static readonly catalogoServicios = [
  ...
  public static obtenerServicios(): Array<{ codigo: string; nombre: string; precio: number; disponible: boolean }> {
    try {
  ...
  public static buscarServicioPorCodigo(codigo: string) {
  ...
  public agregarItemDirecto(codigo: string, descripcion: string, cantidad: number, precioUnitario: number): void {
    this.agregarItem({
      id: this.generarId(),
      tipo: 'impresion',
  ...
  public eliminarItemPorCodigo(codigo: string): void {
