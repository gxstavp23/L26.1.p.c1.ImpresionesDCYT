...
export interface I_vSolicitud {
  ...
  bindVerProductos?(handler: () => void): void;
  bindAgregarProducto?(handler: () => void): void;
  bindEliminarProducto?(handler: () => void): void;
  bindHacerPedido?(handler: () => void): void;
  getCodigoProducto?(): string;
  getCantidadProducto?(): number;
  mostrarProductos?(servicios: { codigo: string; nombre: string; precio: number }[]): void;
  getDatosPago?(): { tipo: string, cedula: string, banco: string | null, telefono: string | null, referencia4: string | null };
